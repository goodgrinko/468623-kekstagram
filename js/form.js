...
(function () {
  ...
  var validateHashTags = function (tags) {
    ...
    for (i = 0; i < tags.length; i++) {
      ...
      // Хэш-теги должны быть разделены пробелами
      if (tags[i].search(/[^А-Яа-яЁёA-Za-z#_\d]/i) !== -1 || tags[i].match(/#/g).length !== 1) {
        return 'Хэш-теги должны быть разделены пробелами';
      }
      // макс длина 20 символов
      if (tags[i].length > 20 || tags[i].length === 1) {
        return 'Длина одного хэш-тега не может быть короче 1 символа и больше 20 символов';
      }
      // макс кол-во 5
      if (tags.length > 5) {
